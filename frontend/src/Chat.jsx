...
function LinkBubble({ href, label }) {
    ...
    return (
        <a href={href} target="_blank" rel="noopener noreferrer"
            style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "2px 10px 2px 7px", background: "rgba(16,163,127,0.1)", border: "1px solid rgba(16,163,127,0.3)", borderRadius: 20, color: "#19c37d", fontSize: 13, fontWeight: 500, textDecoration: "none", cursor: "pointer", verticalAlign: "middle", margin: "1px 3px", maxWidth: 320, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", transition: "all 0.15s" }}
            onMouseEnter={e => { e.currentTarget.style.background = "rgba(16,163,127,0.2)"; e.currentTarget.style.borderColor = "rgba(16,163,127,0.55)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "rgba(16,163,127,0.1)"; e.currentTarget.style.borderColor = "rgba(16,163,127,0.3)"; e.currentTarget.style.transform = "none"; }}
            title={href}>
...
function renderInline(text) {
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        else if (m[4] !== undefined) parts.push(<code key={key++} style={{ fontFamily: "'DM Mono','Fira Mono',monospace", fontSize: "0.84em", padding: "1px 6px", background: "rgba(255,255,255,0.09)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 4, color: "#e5c07b" }}>{m[4]}</code>);
...
function renderMarkdown(rawText) {
    ...
    return segments.map((seg, si) => {
        if (seg.type === "code_block") {
            return (<div key={si} style={{ margin: "10px 0", borderRadius: 8, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", background: "rgba(0,0,0,0.35)" }}>
                ...
                <pre style={{ margin: 0, padding: "12px 16px", fontSize: 13, lineHeight: 1.65, color: "#e5e7eb", fontFamily: "'DM Mono','Fira Mono',monospace", overflowX: "auto", whiteSpace: "pre" }}><code>{seg.content}</code></pre>
            </div>);
        }
        ...
        while (i < lines.length) {
            ...
            if (/^\d+\. /.test(line)) {
                const items = []; let n = 1;
                while (i < lines.length && /^\d+\. /.test(lines[i])) { const content = lines[i].replace(/^\d+\. /, ""); items.push(<div key={i} style={{ display: "flex", gap: 8, marginBottom: 3 }}><span style={{ color: "var(--accent)", fontWeight: 700, minWidth: 18, flexShrink: 0 }}>{n}.</span><span>{renderInline(content)}</span></div>); i++; n++; }
...
function ThinkingIndicator({ botName, action }) {
    ...
    useEffect(() => { const iv = setInterval(() => { setVisible(false); setTimeout(() => { setPhraseIdx(p => (p + 1) % THINKING_PHRASES.length); setVisible(true); }, 200); }, 2200); return () => clearInterval(iv); }, []);
        ...
        <div style={{ display: "flex", padding: "10px 28px", animation: "fadeSlideUp 0.25s ease both" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 13 }}>
                ...
                <div style={{ paddingTop: 3 }}>
                    <div style={{ fontSize: 13, color: "var(--text-secondary)", fontStyle: "italic", opacity: visible ? 1 : 0, transition: "opacity 0.18s ease", marginBottom: 6 }}>{displayText}</div>
...
function useTypewriter(text, speed = 8) {
    ...
    useEffect(() => { if (!text) return; setDisplayed(""); setDone(false); let i = 0; ref.current = setInterval(() => { i++; setDisplayed(text.slice(0, i)); if (i >= text.length) { clearInterval(ref.current); setDone(true); } }, speed); return () => clearInterval(ref.current); }, [text, speed]);
...
function InlineWhatsAppQR() {
    ...
    if (!qrData) return <div style={{ margin: "8px 0", padding: "12px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12 }}><div style={{ fontSize: 13, color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: 8 }}><span style={{ animation: "spin 1s linear infinite", display: "inline-block" }}>⟳</span>{status === "waiting" ? "Activá WhatsAppBot en Bots para generar el QR..." : "Generando QR..."}</div></div>;
    return (
        <div style={{ margin: "8px 0" }}>
            ...
            <div style={{ display: "inline-block", padding: 10, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.4)" }}><img src={qrData.startsWith("data:") ? qrData : `data:image/png;base64,${qrData}`} alt="WhatsApp QR" style={{ width: 200, height: 200, display: "block" }} /></div>
...
function AudioRecorder({ onTranscribed, disabled }) {
    ...
    const startRecording = useCallback(async () => {
        if (recording || processing) return; setFloatError(null);
        try {
            const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
            const mimeType = getMimeType();
            const mr = new MediaRecorder(stream, { mimeType });
            mediaRecorderRef.current = mr; chunksRef.current = []; recordingStartRef.current = Date.now();
    ...
    const stopRecording = useCallback(() => { if (!recording || !mediaRecorderRef.current) return; setRecording(false); mediaRecorderRef.current.stop(); }, [recording]);
    useEffect(() => { const up = () => { if (recording) stopRecording(); }; document.addEventListener("mouseup", up); document.addEventListener("touchend", up); return () => { document.removeEventListener("mouseup", up); document.removeEventListener("touchend", up); }; }, [recording, stopRecording]);
    return (
        <div style={{ position: "relative", flexShrink: 0 }}>
            <button onMouseDown={e => { e.preventDefault(); startRecording(); }} onTouchStart={e => { e.preventDefault(); startRecording(); }} disabled={disabled || processing}
                title={recording ? "Soltá para transcribir" : "Manteé presionado para grabar"}
                style={{ width: 32, height: 32, borderRadius: 8, border: recording ? "1px solid rgba(239,68,68,0.6)" : "1px solid rgba(255,255,255,0.1)", background: recording ? "rgba(239,68,68,0.2)" : processing ? "rgba(16,163,127,0.15)" : "transparent", color: recording ? "#ef4444" : processing ? "var(--accent)" : "var(--text-muted)", cursor: disabled || processing ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, transition: "all 0.15s", animation: recording ? "pulseMic 1s ease-in-out infinite" : "none", userSelect: "none" }}>
                {processing ? <span style={{ animation: "spin 1s linear infinite", display: "inline-block", fontSize: 14 }}>⟳</span> : recording ? "⏹" : "🎤"}
            </button>
            {floatError && <div style={{ position: "absolute", bottom: "calc(100% + 10px)", left: "50%", transform: "translateX(-50%)", background: "rgba(239,68,68,0.95)", backdropFilter: "blur(8px)", color: "#fff", fontSize: 12, padding: "6px 12px", borderRadius: 8, whiteSpace: "nowrap", zIndex: 100, pointerEvents: "none", animation: "fadeSlideUp 0.2s ease both" }}>⚠ {floatError}</div>}
...
function UploadButton({ onUpload, disabled }) {
    ...
    return (<>
        ...
        <button onClick={() => ref.current?.click()} disabled={disabled} title="Adjuntar imagen o PDF" style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "var(--text-muted)", cursor: disabled ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0, transition: "all 0.15s" }}>📎</button>
    </>);
}
...
/* ════════════════════════════════════════════════════════
   HELPERS
═════════════════════════════════════════════════════════ */
function shouldShowQR(msg) { const l = msg.toLowerCase(); return (l.includes("whatsapp") || l.includes("wsp")) && (l.includes("qr") || l.includes("conectar") || l.includes("vincular") || l.includes("escanear")); }
...
function Chat({ propConvId = null }) {
    ...
    // ── Estado del wake word para animar el input ──
    const [wakeWordState, setWakeWordState] = useState("idle"); // "idle" | "listening" | "processing"
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const res = await fetch(`${API}/api/history/conversations/${propConvId}/messages`); const data = await res.json();
            ...
            } catch { setMessages([WELCOME]); } finally { setHistoryLoaded(true); }
        };
        load();
    }, [propConvId]);
    ...
    const handleInputChange = e => { setInput(e.target.value); const el = e.target; el.style.height = "auto"; el.style.height = Math.min(el.scrollHeight, 200) + "px"; };
    ...
    const sendMessage = useCallback(async (text, extra = {}) => {
        ...
        try {
            ...
            addMessage(data.success === false ? "error" : "assistant", data.reply || "Sin respuesta del servidor.", { intent: data.intent, bot: data.bot, ...(wantsQR ? { showQR: true } : {}) });
        } catch (err) { addMessage("error", `Error de conexión: ${err.message}`); }
        setLoading(false); setThinkingBot(null); setThinkingAction(null);
        setTimeout(() => textareaRef.current?.focus(), 50);
    }, [input, loading, conversationId]);

    const handleUpload = async (file) => {
        setUploadLabel(`📎 ${file.name}`); setLoading(true); setThinkingBot("VisionBot"); setThinkingAction("🔍 Analizando con Gemini...");
        ...
        try {
            ...
            const res = await fetch(`${API}/api/upload`, { method: "POST", body: fd }); const data = await res.json();
    ...
    // ── Estilos dinámicos según wake word state ──
    const isListening = wakeWordState === "listening";
    const isProcessingWake = wakeWordState === "processing";
    ...
    return (
        <div className="chat-area">
            <style>{`
                ...
                /* ── Animated border sweep for wake word ── */
                @keyframes wakeListeningBorder {
                ...
                /* Glowing sweeping border: pseudo-element trick */
                .wake-input-box::before {
                    content: '';
                    position: absolute;
                    inset: -1.5px;
                ...
                .wake-input-box.listening::before {
                    opacity: 1;
                    ...
                    animation: wakeListeningBorder 1.6s linear infinite;
                    filter: blur(1px);
                }
                .wake-input-box.processing::before {
                    opacity: 1;
                    background: linear-gradient(90deg,
                        transparent,
                    ...
                    background-size: 300% 100%;
                    animation: wakeListeningBorder 1s linear infinite;
                    filter: blur(1px);
                }

                /* Inner content sits on top of the border animation */
                .wake-input-inner {
                    position: relative;
                    z-index: 1;
                ...
                .wake-input-inner.listening {
                    border: 1px solid rgba(239,68,68,0.5);
                    box-shadow: 0 0 0 1px rgba(239,68,68,0.1), inset 0 0 20px rgba(239,68,68,0.03);
                }
                .wake-input-inner.processing {
                    border: 1px solid rgba(245,158,11,0.5);
                    box-shadow: 0 0 0 1px rgba(245,158,11,0.1);
                }
                .wake-input-inner.idle {
                    border: 1px solid rgba(255,255,255,0.1);
                    box-shadow: none;
                }
                .wake-input-inner.idle:focus-within {
                    border-color: rgba(255,255,255,0.2);
                    box-shadow: 0 0 0 1px rgba(16,163,127,0.15);
                }

                /* ── Watermark text in textarea area ── */
                ...
                .textarea-wrap::after {
                    ...
                    user-select: none;
                    z-index: 0;
                    animation: watermarkPulse 1.4s ease-in-out infinite alternate;
                }
                .textarea-wrap.listening::after { color: rgba(239,68,68,0.18); }
                .textarea-wrap.processing::after { color: rgba(245,158,11,0.18); }
                @keyframes watermarkPulse {
                    from { opacity: 0.5; }
                    to   { opacity: 1; }
                }
                .textarea-wrap textarea { position: relative; z-index: 1; }
            `}</style>
            ...
            <div className="input-area">
                <div style={{ width: "100%", maxWidth: 760 }}>
                    ...
                    <div className={`wake-input-box ${wakeWordState}`}>
                        <div className={`wake-input-inner ${wakeWordState === "idle" ? "idle" : wakeWordState === "listening" ? "listening" : "processing"}`}>
                            ...
                            <button
                                ...
                                style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, border: wakeWordEnabled ? "1px solid rgba(16,163,127,0.4)" : "1px solid rgba(255,255,255,0.1)", background: wakeWordEnabled ? "rgba(16,163,127,0.1)" : "transparent", color: wakeWordEnabled ? "var(--accent)" : "var(--text-muted)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, transition: "all 0.2s" }}
                            >
                                {wakeWordEnabled ? "👂" : "🔕"}
                            </button>

                            {/* Textarea with watermark */}
                            <div
                                className={`textarea-wrap ${isListening ? "listening" : isProcessingWake ? "processing" : ""}`}
                                data-watermark={isListening ? "escuchando..." : isProcessingWake ? "procesando..." : ""}
                            >
                                <textarea
                                    ref={textareaRef}
                                    value={input}
                                    ...
                                    placeholder={isListening || isProcessingWake ? "" : "Escribí un mensaje... o decí 👂 Sistema [comando]"}
                                    disabled={loading}
                                    rows={1}
                                    autoFocus
                                    style={{ background: "transparent", width: "100%", border: "none", outline: "none", color: "var(--text-primary)", fontFamily: "'DM Sans',sans-serif", fontSize: 15, lineHeight: 1.6, resize: "none", minHeight: 24, maxHeight: 200, overflowY: "auto", caretColor: "var(--accent)" }}
                                />
                            </div>
                    ...
                    {/* Wake word — invisible listener, comunica estado al padre */}
                    <WakeWord
